# RTSP Viewer

[English version](https://github.com/leshak/mac-rtsp-viewer/blob/master/README.md)

Нативный просмотрщик RTSP-камер для macOS. Приложение работает из строки меню,
не занимает место в Dock и открывает трансляцию в отдельном окне.

## Возможности

- иконка видеокамеры в строке меню;
- показ и скрытие окна одним кликом;
- воспроизведение `rtsp://` и `rtsps://` через VLCKit;
- RTSP поверх TCP и небольшая задержка буфера для локальных камер;
- ввод адреса вручную, вставка через `⌘V` или кнопку «Вставить»;
- отключение и включение звука одной кнопкой в панели просмотра;
- сохранение адреса между запусками;
- сохранение положения, размера окна и выбранного дисплея между запусками;
- автоматический русский интерфейс для русской локали macOS и английский для
  всех остальных локалей;
- выбор системного, русского или английского языка в настройках;
- настраиваемый HTTP hook для показа и скрытия окна через GET и POST;
- автоматическое скрытие по таймауту и отладочный лог входящих запросов;
- переподключение к потоку без перезапуска приложения;
- понятное сообщение при недоступной камере.

В приложении нет предустановленного адреса камеры. При первом запуске откройте
настройки и укажите собственный RTSP URL.

## Требования

- macOS 15 или новее;
- Xcode 26 или новее;
- интернет при первой сборке: Swift Package Manager скачивает VLCKit/libVLC.

## Быстрый старт

Соберите приложение:

```shell
make app
```

Запустите готовый пакет:

```shell
open "build/RTSP Viewer.app"
```

После запуска:

1. Нажмите иконку видеокамеры в правой части строки меню.
2. Откройте настройки кнопкой с шестерёнкой.
3. Введите или вставьте RTSP URL.
4. При необходимости выберите язык интерфейса вместо системного.
5. Нажмите «Сохранить» — трансляция запустится сразу.

Левый клик по иконке показывает или скрывает окно. Правый клик открывает меню
завершения приложения.

## HTTP hook окна

Встроенный HTTP-сервер слушает все сетевые интерфейсы. По умолчанию endpoint —
`http://<IP-мака>:23456/show/`; он принимает GET и POST:

```shell
curl "http://127.0.0.1:23456/show/?event=motion%20detected"
curl -X POST --data "event=motion detected" "http://127.0.0.1:23456/show/"
```

У GET-запроса имена и значения query-параметров URL-декодируются перед
проверкой правил. У POST BODY проверяется как UTF-8-строка. Порт, сегмент пути,
подстроки для показа и скрытия и таймаут автоскрытия настраиваются в приложении.
Пустая строка показа принимает любой валидный запрос, пустая строка скрытия
отключает это правило. Если совпали оба правила, скрытие имеет приоритет.
Таймаут `0` отключает автоматическое скрытие.

Режим отладки открывает отдельное окно с методом и исходным URL или BODY
входящих запросов. В памяти сохраняются последние 500 запросов.

## Разработка в Xcode

1. Откройте [Package.swift](Package.swift) в Xcode.
2. Выберите схему `RTSPViewer` и назначение `My Mac`.
3. Нажмите Run.

Доступные команды:

| Команда | Назначение |
| --- | --- |
| `make build` | Debug-сборка через Swift Package Manager |
| `make run` | Сборка и запуск из терминала |
| `make app` | Release-сборка пакета `RTSP Viewer.app` |
| `make clean` | Очистка результатов сборки |

Готовое приложение создаётся в `build/RTSP Viewer.app`. Скрипт вкладывает
`VLCKit.framework` в пакет и выполняет локальную ad-hoc подпись.

## Хранение настроек

RTSP URL, выбор языка, состояние звука и конфигурация HTTP hook хранятся в
`UserDefaults` приложения. Кнопка «Очистить» удаляет только сохранённый адрес
камеры. Пароли, указанные непосредственно внутри URL, сохраняются вместе с
адресом.

Вариант «Системный» проверяет предпочитаемую локаль macOS при запуске. Для
русской локали выбирается русский язык, для любой другой — английский.

## Структура проекта

| Путь | Содержимое |
| --- | --- |
| `Sources/RTSPViewer` | AppKit/SwiftUI-код приложения |
| `Support` | Метаданные приложения и локализованные описания разрешений |
| `Scripts/build-app.sh` | Сборка и упаковка `.app` |
| `Package.swift` | Зависимости и настройки Swift Package Manager |

## Видеодвижок и лицензия

Проект использует [VLCKit](https://github.com/videolan/vlckit) и libVLC.
VLCKit/libVLC распространяются на условиях LGPL 2.1. Код этого проекта — на
условиях [MIT License](LICENSE).
